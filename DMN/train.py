...
from tools.calculate_PSNR_SSIM import calc
from models.face_model import Face_Model

os.environ['CUDA_VISIBLE_DEVICES'] = '0'
main_parse = argparse.ArgumentParser()
main_parse.add_argument("--yaml", default='./configs/MRI.yaml', type=str, required=False)
...
def get_train_loader(trainset, world_size, batch_size):
    if world_size <= 1:
        return torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=False, num_workers=CFG.DATA.NUM_WORKERS, pin_memory=False)
    elif world_size > 1:
...
def main(rank, world_size, cpu=False):
    ...
    model.net_load('results/MRI/nets/nets_best.pth')

    trainset = faces_data(data_lr=os.path.join(CFG.DATA.FOLDER, "TRAIN/LR"), data_hr=os.path.join(CFG.DATA.FOLDER, "TRAIN/HR"), img_range=CFG.DATA.IMG_RANGE, rgb=CFG.DATA.RGB)
    ...
    end_ep = int(np.ceil(CFG.OPT.MAX_ITER / (len(loader)+1))) + 1
    ...
    for ep in range(1, end_ep):
        ...
        img_save_folder = os.path.join(CFG.EXP.OUT_DIR, "imgs", "epoch_"+str(ep))
        os.makedirs(img_save_folder, exist_ok=True)
        ...
        if ep % 1 == 0 and rank == last_device:
            ...
            for b in range(len(testset)):
                if b > 1000: break
                    ...
                    save_tensor_image(os.path.join(img_save_folder, f"LR_img_{1+b:d}.jpg"), y, CFG.DATA.IMG_RANGE, CFG.DATA.RGB)
                    # save_tensor_image(os.path.join(img_save_folder, f"{b:04d}_lr.png"), lr, CFG.DATA.IMG_RANGE, CFG.DATA.RGB)
            psnr_step = calc(step=ep)
            psnr.append(psnr_step)
            if psnr_step >= max(psnr):
            	model.net_save_best(net_save_folder)

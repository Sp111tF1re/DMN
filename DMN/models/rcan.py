...
class SALayer(nn.Module):
    ...
    def forward(self, x):
        ...
        y = torch.cat([avg_out, max_out], dim=1)  # 沿着channel维度concat一块
        z = self.conv1(y)
        return x * z
...
def make_SR_net(rgb_range=255, rgb_mean=(0.5, 0.5, 0.5), scale_factor=2 ):
...
if __name__ == "__main__":
    ...
    model1 = make_SR_net(rgb_range=rgb_range, rgb_mean=rgb_mean)
    # print(model)

    X = torch.rand([2, 1, 32, 32], dtype=torch.float32) * rgb_range
    Y = model(X).detach()
    Z = model1(X).detach()
